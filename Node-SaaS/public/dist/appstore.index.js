...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__css_index_scss__ = __webpack_require__(108);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_element_ui__ = __webpack_require__(25);
...
__WEBPACK_IMPORTED_MODULE_3_vue___default.a.use(__WEBPACK_IMPORTED_MODULE_7_element_ui__["Carousel"]);
__WEBPACK_IMPORTED_MODULE_3_vue___default.a.use(__WEBPACK_IMPORTED_MODULE_7_element_ui__["CarouselItem"]);
__WEBPACK_IMPORTED_MODULE_3_vue___default.a.use(__WEBPACK_IMPORTED_MODULE_7_element_ui__["Progress"]);

__WEBPACK_IMPORTED_MODULE_3_vue___default.a.use(__WEBPACK_IMPORTED_MODULE_4_vue_resource__["a" /* default */]);

new __WEBPACK_IMPORTED_MODULE_3_vue___default.a(_defineProperty({
    delimiters: ['[[', ']]'],
    el: '#appstore',
    http: {
        root: '/appstore'
    ...
    created: function created() {
        ...
        _this.$http.get('items').then(function (res) {
            // api2/app/appstore/items
            if (res.status === 200 && res.ok) {
                _this.industryList = res.body.industry;
                _this.categoriesList = res.body.categories;
            }
        });

        _this.$http.get('banner').then(function (res) {
            // api2/app/appstore/banner
            if (res.status === 200 && res.ok) {
                _this.bannerList = res.body;
            }
        });

        _this.$http.get('apps').then(function (res) {
            // api2/app/appstore/apps
            if (res.status === 200 && res.ok) {
                _this.newList = res.body.data.Newest;
                _this.RecommendList = res.body.data.Recommend;
                _this.wholeList = res.body.data.whole;
            }
        });
    },
    watch: {
    ...
    methods: {
        // 显示分类列表
        tabUpdate: function tabUpdate(appId, type) {
            this.isIndex = false;
            if (type) {
                this.localTab = type;
                if (type === 'app') {
                    ...
                    this.appList = this.wholeList;
                } else {
                    this.isIndex = true;
                }
            } else {
                this.localTab = appId; /* appid请求相应分类列表*/
                this.content = appId;
                this.appList = '';
            }
        },
        ...
        showDetails: function showDetails() {
            var dialog = document.getElementById('dialog'),
                app = document.querySelector('.appstore_center'),
            ...
            _this.$http.get('appContent').then(function (res) {
                // api2/app/appstore/appContent
                if (res.status === 200 && res.ok) {
        ...
        // 显示进度条
        showProgress: function showProgress(index) {
            var _this = this;

            query('.button' + index).style.display = 'none';
            query('.progress' + index).style.display = 'block';
            _this.$http.get('progress').then(function (res) {
                // api2/app/appstore/banner
                if (res.status === 200 && res.ok) {
                    _this.percent = res.body.result * 100;
                    if (_this.percent === 100) {
                        query('.button' + index).style.display = 'block';
                        query('.progress' + index).style.display = 'none';
                    }
                }
            });
        },
